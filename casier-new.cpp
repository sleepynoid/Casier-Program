...
bool addsItem(string namaBarang, goodsSale (&barang)[10]) {
  ...
  if (tmpIndex != -1 && tmpJumlahMax != 0) {
    do {
      printf("Masukan jumlah %s yang anda beli [%d] : ",
             barang[tmpIndex].nama.c_str(), tmpJumlahMax);
      cin >> tmpJumlah;
      if (tmpJumlah > tmpJumlahMax)
        printf("Maaf Jumlah Pembelian Melebihi Stock\n");
      else if (tmpJumlah < 0)
        printf("Masukan Jumlah dengan Benar\n");
...
void removeItem(string namaBarang, goodsSale (&barang)[10]) {
  ...
  if (tmpIndex != -1 && tmpJumlahMax != 0) {
    do {
      printf("Masukan jumlah %s yang anda hapus [%d] : ",
             barang[tmpIndex].nama.c_str(), barang[tmpIndex].jumlah);
      cin >> tmpJumlah;
    } while (tmpJumlah > tmpJumlahMax || tmpJumlah < 0);
    barang[tmpIndex].jumlah -= tmpJumlah;
  }
}
void view_barang(goodsSale (&barang)[10]) {
  printf("|Nama Barang|\tHarga barang|\tStok Barang|\n");
  for (int i = 0; i <= Index; i++) {
    printf("%s\t\t %d\t\t %d\t\t %d\n", barang[i].nama.c_str(), barang[i].stock,
           barang[i].jumlah, barang[i].harga);
  }
  printf("--------------------------------------------\n");
}
...
void addGoods(string namaBarang, int harga, int stock,
              struct goodsSale (&barang)[10]) {
  int tempIndex = ++Index;
...
void checkout(goodsSale (&barang)[10]) {
  ...
  int total = 0, tunai = 0;
  for (int i = 0; i <= find; i++) {
    printf("barang 1=%d barang 2=%d", barang[tmpIndex[find]].harga,
           barang[tmpIndex[find]].jumlah);
